...
ARadarVolumeRender::ARadarVolumeRender()
{
	...
	loadingTextComponent = CreateDefaultSubobject<UTextRenderComponent>(TEXT("LoadingText"));
	loadingTextComponent->SetupAttachment(RootComponent);
	loadingTextComponent->SetRelativeLocation(FVector(0, 0, 0.65f));
	loadingTextComponent->SetUsingAbsoluteScale(true);
	loadingTextComponent->SetWorldSize(50.0f);
	loadingTextComponent->SetTextRenderColor(FColor::Yellow);
	loadingTextComponent->SetText(FText::FromString(TEXT("LOADING DATA...")));
	loadingTextComponent->SetHorizontalAlignment(EHTA_Center);
...
void ARadarVolumeRender::Tick(float DeltaTime)
{
	...
	if (loadingTextComponent && loadingTextComponent->IsVisible()) {
		APlayerCameraManager* camManager = UGameplayStatics::GetPlayerCameraManager(this, 0);
		if (camManager) {
			FVector camLoc = camManager->GetCameraLocation();
			FVector textLoc = loadingTextComponent->GetComponentLocation();
			FRotator textRot = (camLoc - textLoc).Rotation();
			loadingTextComponent->SetWorldRotation(textRot);
			
			// Dynamically scale text so it's always readable regardless of map scale
			float dist = FVector::Dist(camLoc, textLoc);
			float newScale = FMath::Max(0.5f, dist / 800.0f);
