...
void ASettingsSaver::BeginPlay(){
	Super::BeginPlay();
	settingsFile = FPaths::Combine(FPaths::Combine(FPlatformProcess::UserSettingsDir(), TEXT("NexradSpatial")), TEXT("settings.json"));
	locationMarkersFile = FPaths::Combine(FPaths::Combine(FPlatformProcess::UserSettingsDir(), TEXT("NexradSpatial")), TEXT("location-markers.json"));
...
void ASettingsSaver::LoadSettings() {
	if (ARadarGameStateBase* gameState = GetWorld()->GetGameState<ARadarGameStateBase>()) {
		...
		LOAD_MACRO_BOOL(showTornadoWarnings);
		LOAD_MACRO_BOOL(showSevereWarnings);
		LOAD_MACRO_BOOL(showFlashFloodWarnings);
		LOAD_MACRO_BOOL(showMarineWarnings);
		...
		LOAD_MACRO_BOOL(enableAdaptiveVolumetricThrottle);
...
void ASettingsSaver::SaveSettings() {
	if (ARadarGameStateBase* gameState = GetWorld()->GetGameState<ARadarGameStateBase>()) {
		...
		SAVE_MACRO_BOOL(showTornadoWarnings);
		SAVE_MACRO_BOOL(showSevereWarnings);
		SAVE_MACRO_BOOL(showFlashFloodWarnings);
		SAVE_MACRO_BOOL(showMarineWarnings);
		...
		SAVE_MACRO_BOOL(enableAdaptiveVolumetricThrottle);
...
void ASettingsSaver::ResetBasicSettings() {
	if (ARadarGameStateBase* gameState = GetWorld()->GetGameState<ARadarGameStateBase>()) {
		...
		RESET_MACRO(enableAdaptiveVolumetricThrottle);
...
void ASettingsSaver::ResetAllSettings() {
	if (ARadarGameStateBase* gameState = GetWorld()->GetGameState<ARadarGameStateBase>()) {
		...
		RESET_MACRO(showTornadoWarnings);
		RESET_MACRO(showSevereWarnings);
		RESET_MACRO(showFlashFloodWarnings);
		RESET_MACRO(showMarineWarnings);
		RESET_MACRO(mapBrightness);
